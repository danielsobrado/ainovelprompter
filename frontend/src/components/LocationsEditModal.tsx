...
interface LocationsEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  options: LocationOption[];
  onSave: (options: LocationOption[]) => void;
}

export default function LocationsEditModal({
  isOpen,
  onClose,
  options,
  onSave,
}: LocationsEditModalProps) {
  // State management
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Edit Locations</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={selectedItemId} onValueChange={setSelectedItemId}>
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Select or create new location" />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex space-x-2">
            <Button onClick={handleSave} className="flex-1">
              {selectedItemId === 'new' ? 'Add Location' : 'Update Location'}
            </Button>
            {selectedItemId !== 'new' && (
